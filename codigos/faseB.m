...
[pi_hibrido,pi_flow,pi_volt] = contingencia_mais_severa(sistema2);
qtd_linhas_sistema = size(sistema2.branch);
tam_pi = size(pi_hibrido);
...
for m = 1:tam_pi
    ...
    for n = 1:tam_sobrecar
        if pi_hibrido(m,1) == sobrecar(n,1) && pi_hibrido(m,2) == sobrecar(n,2)
            sair = 0;
...
while sair ~= 1
    for m = 1:qtd_linhas_sistema
        if sistema2.branch(m,1) == pi_hibrido(indice_pi,1) && sistema2.branch(m,2) == pi_hibrido(indice_pi,2)
    ...
    if resultados.success == 0 || tam_sobrecar(1) == 0
        for m = 1:qtd_linhas_sistema
            ...
            elseif sistema2.branch(m,1) == pi_hibrido(indice_pi+1,1) && sistema2.branch(m,2) == pi_hibrido(indice_pi+1,2)
                sistema2.branch(m,11) = 0;
                sistema1.branch(m,11) = 0;
                 linha_removida(1) = sistema2.branch(m,1);
                 linha_removida(2) = sistema2.branch(m,2);
...
clc
fprintf('CENÁRIO 1B\n');
fprintf('Determinar solução do FPORS para o Cenário 1B?\n')
fprintf('[1] Sim\n');
fprintf('[2] Não\n');
fprintf('[0] Voltar\n');
opcao = input('-> ');
while opcao ~= 0
    ...
    if(opcao ~= 0)
        clc
        fprintf('CENÁRIO 1B\n');
        fprintf('Determinar solução do FPORS para o Cenário 1?\n')
        fprintf('[1] Sim\n');
        fprintf('[2] Não\n');
        fprintf('[0] Voltar\n');
        opcao = input('-> ');
